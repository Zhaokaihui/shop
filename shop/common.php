...
/**
 * *
 * 版权所有 2016-2066 广州阳光信息科技有限公司，并保留所有权利。
 *
 *
 * * * * * */
...
function WSTConf($key,$v = ''){
	if(is_null($v)){
		if(array_key_exists('shopCONF',$GLOBALS) && array_key_exists($key,$GLOBALS['shopCONF'])){
		    unset($GLOBALS['shopCONF'][$key]);
		}
	}else if($v === ''){
		if(array_key_exists('shopCONF',$GLOBALS)){
			$conf = $GLOBALS['shopCONF'];
			$ks = explode(".",$key);
...
function WSTAutoRegister($loginSrc = 0){
    ...
    if($loginSrc == 5){
        ...
        $openId = session('tb_WX_OPENID')?session('tb_WX_OPENID'):'';
        if($openId!=''){
            $user = Db::name('users')->where([["wxOpenId",'=',$openId],['dataFlag','=',1]])->find();
        }
        $userinfo = session('tb_WX_USERINFO');
		if($userinfo){
...
function WSTRecordResources($resPath, $fromType, $resType=0){
	...
	//根据类型判断所有者
	$data['ownId'] = ((int)$fromType==0)?(int)session('tb_USER.userId'):(int)session('tb_STAFF.staffId');
	$data['isUse'] = 0; //默认不使用
	$data['createTime'] = date('Y-m-d H:i:s');
...
function WSTUseResource($fromType, $dataId, $resPath, $fromTable='', $resFieldName=''){
	...
	//根据类型判断所有者
	$image['ownId'] = ((int)$fromType==0)?(int)session('tb_USER.userId'):(int)session('tb_STAFF.staffId');
...
function WSTLangPayFrom($pkey = '',$type = 0){
    $paySrc = cache('tb_PAY_SRC');
    if(!$paySrc){
        $paySrc = Db::name('payments')->order('payOrder asc')->select();
        cache('tb_PAY_SRC',$paySrc,31622400);
    }
...
function WSTMsgTemplates($tplCode){
	...
	if(!$data){
		...
		cache('tb_MSG_TEMPLATES',$data,378432000);
	}
...
function WSTWxMessage($params){
	...
	if($tpl && file_exists('shop'.DS.'wechat'.DS.'behavior'.DS.'InitWechatMessges.php')){
		Hook::exec(['shop\\wechat\\behavior\\InitWechatMessges','run'],$params);
	}
}
/**
 * 批量发送微信消息
...
function WSTWxBatchMessage($params){
    $tpl = WSTMsgTemplates($params['CODE']);
	if($tpl && file_exists('shop'.DS.'wechat'.DS.'behavior'.DS.'InitWechatMessges.php')){
		\think\facade\Hook::exec(['shop\\wechat\\behavior\\InitWechatMessges','batchRun'],$params);
	}
}
/**
 * 发送小程序消息
...
function WSTWeMessage($params){
    $tpl = WSTMsgTemplates($params['CODE']);
	if($tpl && file_exists('shop'.DS.'weapp'.DS.'behavior'.DS.'InitWeappMessges.php')){
		Hook::exec(['shop\\weapp\\behavior\\InitWeappMessges','run'],$params);
	}
}
/**
 * 截取字符串
...
function WSTClearHookCache(){
	...
	if(!empty($STAFF)){
	    //获取角色权限
	 	...
	 	session('tb_STAFF',$STAFF);
	}
	WSTConf('protectedUrl',null);
	cache('tb_HOME_MENUS',null);
	cache('tb_PRO_MENUS',null);
	cache('tb_MOBILE_BTN',null);
	cache('hooks',null);
	cache('tb_ADDONS',null);
	WSTConf('tb_ADDONS',null);
}
/**
 * 获取移动端首页按钮
 */
function WSTMobileBtns($src){
    $data = cache('tb_MOBILE_BTN');
    if(!$data){
        ...
        cache('tb_MOBILE_BTN',$data,31536000);
    }
...
function WSTShopOrderMenus(){
	...
	if(isset($tb_user['shopId'])){
		$shop = model('common/Shops')->getFieldsById($tb_user['shopId'],'shopStatus');
...
function WSTHomeMenus($menuType){
    ...
    //判断menuId1是否有效
    if($menuId1==0){
        $menuId1 = (int)session('tb_MENID'.$menuType);
    }else{
        session('tb_MENID'.$menuType,$menuId1);
    }
    //检测第一级菜单是否有效
    $tmpMenuId1 = 0;
    $isFind = false;
    ...
    $menus['menuId3'] = session('tb_MENUID3');
    return $menus;
}
...
function WSTSigninfo($info,$user){
	if(!empty($info['openid'])){
		...
		session('tb_WX_SIGNINFO',$signinfo);
	}
}

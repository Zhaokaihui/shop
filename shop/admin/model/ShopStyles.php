    ...
    public function initStyles(){
        ...
        foreach ($styles as $key => $v) {
            ...
            if($v['stylePath'] != "shop_home" && $v['stylePath'] != "self_shop"){
                ...
                if($v['styleSys'] != 'weapp' && $v['styleSys'] != 'app'){
                    //删除不存在的风格记录
                    if(!is_dir(WSTRootPath(). DS .'shop'. DS .$v['styleSys']. DS .'view'. DS.$stylePath)){
        ...
        try{
            ...
            foreach ($sys as $key => $v) {
                $dirs = array_map('basename',glob(WSTRootPath(). DS .'shop'.DS.$key.DS.'view/default/theme/shop'.DS.'*', GLOB_ONLYDIR));
                foreach ($dirs as $dkey => $dv) {
                    $path = "theme/shop/".$dv."/shop_home";
                    if(!isset($v[$path])){
                        $sqlPath = WSTRootPath(). DS .'shop'. DS .$key. DS .'view/default/theme/shop'. DS .$dv. DS.'sql'.DS.'install.sql';// sql路径

...
use shop\admin\model\Roles;
use shop\admin\model\LogStaffLogins;
use shop\admin\validate\Staffs as validate;
use think\Db;
/**
 * * * * * * * 职员业务处理
 */
class Staffs extends Base{
	 ...
	 */
    public function del(){
    	$staffId = (int)session('tb_STAFF.staffId');
	...
	public function edit(){
		...
		try{
	        ...
	        if(false !== $result){
		        Db::commit();
		        if(session('tb_STAFF.staffId')==$id){
		        session('tb_STAFF.staffPhoto',$data['staffPhoto']);
		        session('tb_STAFF.staffName',$data['staffName']);
		    }

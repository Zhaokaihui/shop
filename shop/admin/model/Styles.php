...
class Styles extends Base{
	...
	public function initStyles(){
		...
		foreach ($styles as $key => $v) {
			...
			//删除不存在的风格记录
			if(!is_dir(WSTRootPath(). DS .'shop'. DS .$v['styleSys']. DS .'view'. DS.$v['stylePath'])){
        ...
        try{
			...
			foreach ($sys as $key => $v) {
			    $dirs = array_map('basename',glob(WSTRootPath(). DS .'shop'.DS.$key.DS.'view'.DS.'*', GLOB_ONLYDIR));
		        foreach ($dirs as $dkey => $dv) {
		        	if(!isset($v[$dv])){
		        		$sqlPath = WSTRootPath(). DS .'shop'. DS .$key. DS .'view'. DS .$dv. DS.'sql'.DS.'install.sql';// sql路径
	...
	public function changeStyle(){
		 ...
		 $sqlPath = WSTRootPath(). DS .'shop'. DS .$object['styleSys']. DS .'view'. DS .$object['stylePath']. DS.'sql'.DS.'style.sql';// sql路径
		 $hasFile = file_exists($sqlPath);
    ...
    */
    private function excute($sql,$db_prefix=''){
		...
		$sql = str_replace("\r", " ", str_replace('`tb_', '`'.$db_prefix, $sql));// 替换表前缀
		$ret = array();
		foreach(explode(";\n", trim($sql)) as $query) {

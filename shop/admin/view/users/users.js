...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'昵称', name:'userName' ,width:150,sortable:true,renderer:function(val,item,rowIndex){
                var html = '';
                if(item['wxOpenId'] && item['wxOpenId']!='')html = html+'<img class="order-source2" src="'+WST.conf.ROOT+'/shop/admin/view/img/order_source_1.png" title="有绑定微信"> ';
                if(item['weOpenId'] && item['weOpenId']!='')html = html+'<img class="order-source2" src="'+WST.conf.ROOT+'/shop/admin/view/img/order_source_5.png" title="有绑定小程序"> ';
            ...
            {title:'操作', name:'' ,width:150, align:'center', renderer: function(val,rowdata,rowIndex){
                var h = "";
                if(WST.GRANT.HYGL_02)h += "<a  class='btn btn-blue' href='"+WST.U('admin/Users/toEdit','id='+rowdata['userId'])+'&p='+tb_CURR_PAGE+"'><i class='fa fa-pencil'></i>修改</a> ";
                ...
                h += "<br/><a href='"+WST.U('admin/userscores/touserscores','id='+rowdata['userId'])+'&p='+tb_CURR_PAGE+"'>积分</a> ";
                h += "<a href='"+WST.U('admin/logmoneys/tologmoneys','id='+rowdata['userId']+'&src=users')+'&p='+tb_CURR_PAGE+"&type=0'>用户资金</a> ";
                h += "<a href='"+WST.U('admin/orders/index','userId='+rowdata['userId'])+'&p='+tb_CURR_PAGE+"&type=0'>消费信息</a> ";
                return h;
            }}
            ];
...
function toEdit(id){
   location.href=WST.U('admin/users/toEdit','id='+id+'&p='+tb_CURR_PAGE);
}

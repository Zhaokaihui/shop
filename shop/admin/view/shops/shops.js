...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'操作', name:'' ,width:150, align:'center', renderer: function(val,item,rowIndex){
	            ...
	            h += "<a class='btn btn-blue' href='"+WST.U('admin/logmoneys/tologmoneys','id='+item['shopId']+'&src=shops&p='+tb_CURR_PAGE)+"&type=1'><i class='fa fa-search'></i>商家资金</a>";
	            return h;
            }}
            ];
...
function toHandle(id){
	location.href = WST.U('admin/shops/toHandleApply','id='+id+'&p='+tb_CURR_PAGE);
}
	...
	var uploader = WST.upload({
				...
				callback:function(f,file){
					...
					if(json.status==1){
						...
						var btn = $('<div style="position: absolute;top: -5px;right: 0px;cursor: pointer;background: rgba(0,0,0,0.5);width: 18px;height: 18px;text-align: center;border-radius: 50%;" ><img src="'+WST.conf.ROOT+'/shop/home/View/default/img/seller_icon_error.png"></div>');
						tdiv.append(btn);
...
function toEdit(id,src){
	location.href=WST.U('admin/shops/toEdit','id='+id+'&p='+tb_CURR_PAGE+'&src='+src);
}

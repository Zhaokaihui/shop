...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'操作', name:'' ,width:100, align:'center', renderer: function(val,item,rowIndex){
                var h = "";
                if(WST.GRANT.DPBZ_02)h += "<a  class='btn btn-blue' onclick='javascript:toEdit("+item['articleId']+")'><i class='fa fa-pencil'></i>修改</a> ";
                if(WST.GRANT.DPBZ_03)h += "<a  class='btn btn-red' onclick='javascript:toDel(" + item['articleId'] + ")'><i class='fa fa-trash-o'></i>删除</a> ";
                return h;
            }}
            ];
...
function toEdit(id){
	location.href=WST.U('admin/articles/toEditHelp','id='+id+'&p='+tb_CURR_PAGE);
}

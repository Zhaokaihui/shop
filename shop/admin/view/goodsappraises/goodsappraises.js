...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'商品评分', name:'goodsScore',sortable: true, width: 80, renderer: function(val,item,rowIndex){
	        	...
	        	for(var i=0;i<val;++i){
	        		s +="<img src='"+WST.conf.ROOT+"/shop/admin/view/goodsappraises/icon_score_yes.png'>";
	        	}
	        	s += "</div>";
	        	return s;
            }},
            ...
            {title:'操作', name:'' ,width:95, align:'center', renderer: function(val,item,rowIndex){
                var h = "";
	            if(WST.GRANT.PJGL_02)h += "<a class='btn btn-blue' href='"+WST.U('admin/goodsappraises/toEdit','id='+item['id'])+'&p='+tb_CURR_PAGE+"'><i class='fa fa-pencil'></i>修改</a> ";

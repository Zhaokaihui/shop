...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'投诉订单号', name:'orderNo',sortable: true, renderer: function(val,item,rowIndex){
            	var h = "";
	            h += "<img class='order-source2' src='"+WST.conf.ROOT+"/shop/admin/view/img/order_source_"+item['orderSrc']+".png'>"; 
            ...
            {title:'被投诉人',width: 30,sortable: true, name:'supplierName'},
...
function toView(id){
	location.href=WST.U('admin/SupplierOrderComplains/view','cid='+id+'&p='+tb_CURR_PAGE);
}
function toHandle(id){
	location.href=WST.U('admin/SupplierOrderComplains/toHandle','cid='+id+'&p='+tb_CURR_PAGE);
}
...
function showDetail(id){
    parent.showBox({title:'订单详情',type:2,content:WST.U('admin/SupplierOrderComplains/view',{cid:id,from:1}),area: ['1020px', '500px'],btn:['关闭']});
}

...
function initGrid(p){
  ...
  var cols = [
            ...
            {title:'操作', name:'' ,width:60, align:'center', renderer: function(val,item,rowIndex){
                var h="";
	            if(WST.GRANT.GRZLC_02)h += "<a  class='btn btn-blue' href='javascript:toEditFlow(" + item['flowId'] + ")'><i class='fa fa-search'></i>查看</a> ";
	            if(WST.GRANT.GRZLC_02)h += "<a  class='btn btn-blue' href='javascript:getForEdit(" + item['flowId'] + ")'><i class='fa fa-pencil'></i>修改</a> ";

	            if(WST.GRANT.GRZLC_03 && item['isDelete'] == 1)h += "<a  class='btn btn-red' href='javascript:toDel(" + item['flowId'] + ")'><i class='fa fa-trash-o'></i>删除</a> ";
	            return h;
            }}
            ];

	...
	var box = WST.confirm({content:"您确定要提醒发货吗?",yes:function(){
			...
			$.post(WST.U('shop/supplierorders/noticeDeliver'),{id:id},function(data){
				...
				if(json.status>0){
					...
					waitReceiveByPage(tb_CURR_PAGE);
				    layer.close(ll);
				}else{
		        ...
		        yes: function(index, layero){
				    ...
				    $.post(WST.U('shop/Supplierorderrefunds/refund'),params,function(data){
						...
						if(json.status==1){
							...
							layer.close(w);
							if(src=='abnormal'){
                                abnormalByPage(tb_CURR_PAGE);
							}else{
                                cancelByPage(tb_CURR_PAGE);
							}
						}else{
							WST.msg(json.msg, {icon: 2});
						}
...
function view(id,src){
	location.href=WST.U('shop/supplierorders/detail','id='+id+'&src='+src+'&p='+tb_CURR_PAGE);
}
function complain(id,src){
	location.href=WST.U('shop/supplierordercomplains/complain','orderId='+id+'&src='+src+'&p='+tb_CURR_PAGE);
}
function afterSale(id,src){
	location.href=WST.U('shop/supplierorderservices/index','orderId='+id+'&src='+src+'&p='+tb_CURR_PAGE);
}
...
function upload(n){
    var uploader =WST.upload({
        ...
        callback:function(f,file){
          ...
          var btn = $('<div style="position:relative;top:-80px;left:60px;cursor:pointer;" ><img src="'+WST.conf.ROOT+'/shop/shop/view/default/img/seller_icon_error.png"></div>');
          tdiv.append(btn);
...
function userAppraise(p){
  $('#list').html('<img src="'+WST.conf.ROOT+'/shop/shop/view/default/supplier/img/loading.gif">正在加载数据...');
  var params = {};
  params = WST.getParams('.s-query');
  params.key = $.trim($('#key').val());
  params.page = p;
...
/*********************** 用户投诉列表页面 ***************************/
function toView(id){
  location.href=WST.U('shop/supplierordercomplains/getUserComplainDetail','id='+id+'&p='+tb_CURR_PAGE);
}

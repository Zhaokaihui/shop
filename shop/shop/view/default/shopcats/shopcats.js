...
function treeCatOpen(obj,id){
    if( $(obj).attr('class').indexOf('active') > -1 ){
        ...
        $(obj).html('<img class="wst-lfloat" style="margin-top:-3px;" src="'+WST.conf.ROOT+'/shop/shop/view/default/img/seller_icon_sq.png">');
        $('.tree_'+id).hide();
    }else{
    	$(obj).addClass('active');
        $(obj).html('<img class="wst-lfloat" style="margin-top:-3px;" src="'+WST.conf.ROOT+'/shop/shop/view/default/img/seller_icon_zk.png">');
        $('.tree_'+id).show();
    }
}
...
function batchSaveCats(){
	...
	$.post(WST.U('shop/shopcats/batchSaveCats'),params,function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status==1){
...
function editCatName(obj){
	$.post(WST.U('shop/shopcats/editName'),{"id":$(obj).attr('dataId'),"catName":obj.value},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
...
function editCatSort(obj){
	$.post(WST.U('shop/shopcats/editSort'),{"id":$(obj).attr('dataId'),"catSort":obj.value},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
			WST.msg('操作成功!',{icon: 1,time:500});
		}else{

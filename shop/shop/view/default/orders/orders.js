					...
					WST.confirm({content:'您确定修改后的订单价格为¥<span class="j-warn-order-money">'+newOrderMoney+'</span>吗？',yes:function(cf){
						...
						$.post(WST.U('shop/orders/editOrderMoney'),{id:id,orderMoney:newOrderMoney},function(data){
							...
							if(json.status>0){
								...
								waituserPayByPage(tb_CURR_PAGE);
								layer.close(cf);
								layer.close(index);
						    	layer.close(ll);
							}else{
	...
	if(deliverType==1){
        WST.confirm({content:"您确定用户已提货了吗？", yes:function(tips){
            ...
            $.post(WST.U('shop/orders/deliver'),{id:id,expressId:0,expressNo:''},function(data){
				...
				if(json.status>0){
					WST.msg(json.msg,{icon:1});
					waitDivleryByPage(tb_CURR_PAGE);
					layer.close(tips);
				    layer.close(ll);
				}else{
					WST.msg(json.msg,{icon:2});
...
function view(id,src){
	location.href=WST.U('shop/orders/view','id='+id+'&src='+src+'&p='+tb_CURR_PAGE);
}


/********** 订单投诉列表 ***********/
function toView(id){
  location.href=WST.U('shop/ordercomplains/getShopComplainDetail','id='+id+'&p='+tb_CURR_PAGE);
}
function toBack(p){
...
function toRespond(id){
  location.href=WST.U('shop/ordercomplains/respond','id='+id+'&p='+tb_CURR_PAGE);
}

function complainByPage(p){
  $('#list').html('<img src="'+WST.conf.ROOT+'/shop/shop/view/default/img/loading.gif">正在加载数据...');
  var params = {};

...
use shop\common\model\Orders as OM;
use shop\common\model\LogMoneys as LM;
use shop\common\model\Shops as SM;
use shop\common\model\Suppliers as SUM;
use shop\common\model\LogPays as PM;
...
class Weixinpays extends Base{
	...
	public function getWeixinPaysURL(){
		...
		if($payObj=="recharge"){
			...
			$targetId = (int)session('tb_USER.userId');
			if($targetType==1){//商家
				$targetId = (int)session('tb_USER.shopId');
			}
			$needPay = 0;
			if($itmeId>0){
	...
	public function getPayStatus() {
		...
		$obj = array();
		$obj["userId"] = (int)session('tb_USER.userId');

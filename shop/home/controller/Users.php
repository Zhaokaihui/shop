...
class Users extends Base{
	...
	public function toRegist(){
		...
		$m = new MUsers();
		$rs = $m->regist();
		$rs['url'] = session('tb_HO_CURRENTURL');
		return $rs;
	...
	public function getPhoneVerifyCode(){
		...
		$m = new MUsers();
		$rs = $m->checkUserPhone($userPhone,(int)session('tb_USER.userId'));
		if($rs["status"]!=1){
	...
	public function index(){
		session('tb_MENID0',0);
		session('tb_MENUID30',0);
        // 待付款 待收货 待评价
        $info = model('home/Users')->getStatusNum();
        $this->assign($info);
        $m = new MUsers();
        $data = $m->getById((int)session('tb_USER.userId'));
    ...
    */
    ...
    public function getEmailVerify(){
    	...
    	$rs = WSTCheckLoginKey($userEmail,(int)session('tb_USER.userId'));
    	if($rs["status"]!=1){
        ...
        if( $tpl['tplContent']!='' && $tpl['status']=='1'){
            ...
            $replace = [session('tb_USER.loginName'),date('Y-m-d H:i:s'),$code,30];
    ...
    public function emailEdit(){
    	...
    	$rs = WSTCheckLoginKey($bindEmail,(int)session('tb_USER.userId'));
    	...
    	$m = new MUsers();
    	$rs = $m->editEmail((int)session('tb_USER.userId'),$bindEmail);
    	if($rs['status'] == 1){
            // 清空session
            session('email',null);
    ...
    */
    ...
    public function getEmailVerifyt(){
    	$m = new MUsers();
    	$data = $m->getById(session('tb_USER.userId'));
        ...
        if($sendRs['status']==1){
            // 修改的用户
            session('email.uId',(int)session('tb_USER.userId'));
            // 绑定的邮箱
            session('email.val',$userEmail);
            // 验证码
            session("email.key", $code);
    ...
    public function emailEditt(){
    	...
    	$m = new MUsers();
    	$data = $m->getById($uId);
    	if($data['userId']==session('tb_USER.userId')){
    		return WSTReturn('验证通过',1);
    	}
        $this->error('无效的用户！');
    }
    ...
    */
    ...
    public function getPhoneVerifyo(){
    	...
    	$rs = array();
    	$m = new MUsers();
    	$rs = WSTCheckLoginKey($userPhone,(int)session('tb_USER.userId'));
    	if($rs["status"]!=1){
    		return WSTReturn("手机号已存在!");
    		exit();
    	}
    ...
    public function phoneEdito(){
    	...
    	if(!session('Verify_info.phoneVerify') || time()>floatval($timeVerify)+10*60){
   		...
   		if($phoneVerify==session('Verify_info.phoneVerify')){
   			$m = new MUsers();
   			$rs = $m->editPhone((int)session('tb_USER.userId'),session('Verify_info.userPhone'));
   			if($process=='Two'){
   				$rs['process'] = $process;
   			}else{
   				$rs['process'] = '0';
    ...
    */
    ...
    public function getfindPhone(){
    	session('tb_USER',session('findPass.userId'));
    ...
    */
    ...
    public function getUserMoney(){
        $m = new MUsers();
        $rs = $m->getFieldsById((int)session('tb_USER.userId'),'userMoney,lockMoney,rechargeMoney,payPwd');

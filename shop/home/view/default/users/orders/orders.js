...
function view(id,src){
	location.href=WST.U('home/orders/detail','id='+id+'&src='+src+'&p='+tb_CURR_PAGE);
}
function complain(id,src){
	location.href=WST.U('home/ordercomplains/complain','orderId='+id+'&src='+src+'&p='+tb_CURR_PAGE);
}
function afterSale(id,src){
	location.href=WST.U('home/orderservices/index','orderId='+id+'&src='+src+'&p='+tb_CURR_PAGE);
}
...
function upload(n){
    var uploader =WST.upload({
        ...
        callback:function(f,file){
          ...
          var btn = $('<div style="position:relative;top:-80px;left:60px;cursor:pointer;" ><img src="'+WST.conf.ROOT+'/shop/home/view/default/img/seller_icon_error.png"></div>');
          tdiv.append(btn);
...
function userAppraise(p){
  $('#list').html('<img src="'+WST.conf.ROOT+'/shop/home/view/default/img/loading.gif">正在加载数据...');
  var params = {};
  params = WST.getParams('.s-query');
  params.key = $.trim($('#key').val());
  params.page = p;
...
/*********************** 用户投诉列表页面 ***************************/
function toView(id){
  location.href=WST.U('home/ordercomplains/getUserComplainDetail','id='+id+'&p='+tb_CURR_PAGE);
}

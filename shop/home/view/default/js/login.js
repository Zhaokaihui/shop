...
function getVerifyCode(type){
	...
	if(window.conf.SMS_VERFY=='1'){
		var html = [];
			...
			html.push('<img style="vertical-align:middle;cursor:pointer;height:30px;" class="verifyImgd" src="'+WST.DOMAIN+'/shop/Home/View/default/images/clickForVerify.png" title="刷新验证码" onclick="javascript:WST.getVerify(\'.verifyImgd\',\'#smsVerfyl\')"/>');
...
function initRegist(){
	...
	$('#reg_form').validator({
	    ...
	    valid: function(form){
	        ...
	        me.holdSubmit();
	        var params = WST.getParams('.tb_ipt');

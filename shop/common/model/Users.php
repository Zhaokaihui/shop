...
class Users extends Base{
    ...
    public function checkLogin($loginSrc = 0){
    	...
    	if(!empty($rs)){
    		...
    		$wxOpenId = session('tb_WX_OPENID');
    		if($wxOpenId){
    			$update['wxOpenId'] = $rs['wxOpenId'] = session('tb_WX_OPENID');
    ...
    public function regist($loginSrc = 0){
        ...
        $wxOpenId = session('tb_WX_OPENID');
    	if($wxOpenId){
    		$data['wxOpenId'] = session('tb_WX_OPENID');
			$userinfo = session('tb_WX_USERINFO');
			if($userinfo){
    ...
    */
    public function edit($userId=0){
    	$Id = ($userId>0)?$userId:(int)session('tb_USER.userId');
    	$data = input('post.');
        unset($data['userId']);

...
class Orders extends Base{
	...
	public function getByView($orderId, $uId=0){
		$userId = ($uId==0)?(int)session('tb_USER.userId'):$uId;
		$shopId = ($uId==0)?(int)session('tb_USER.shopId'):$uId;
	...
	*/
	...
	public function toExport(){
		...
		// 设置excel文档的属性
		$objPHPExcel->getProperties()->setCreator("shop")//创建人
		->setLastModifiedBy("shop")//最后修改人
		->setTitle($name)//标题
		->setSubject($name)//题目
		...
		try{
			...
			if($rs!==false){
                ...
                if( $tpl['tplContent']!='' && $tpl['status']=='1'){
                    ...
                    $replace = [session('tb_USER.loginName'),$info['orderNo']];

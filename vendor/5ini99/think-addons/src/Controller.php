...
class Controller extends \think\Controller
{
    ...
    public function __construct()
    {
        ...
        $this->filter(function($content){
            ...
            $content = str_replace("__SHOP__",str_replace('/index.php','',Request::root()).'/shop/shop/view/default',$content);
            $content = str_replace("__STYLE__",str_replace('/index.php','',Request::root()).'/shop/home/view/'.WSTConf('CONF.wsthomeStyle'),$content);
            $content = str_replace("__ADMIN__",str_replace('/index.php','',Request::root()).'/shop/admin/view',$content);
            $content = str_replace("__WECHAT__",str_replace('/index.php','',Request::root()).'/shop/wechat/view/'.WSTConf('CONF.wstwechatStyle'),$content);
            $content = str_replace("__MOBILE__",str_replace('/index.php','',Request::root()).'/shop/mobile/view/'.WSTConf('CONF.wstmobileStyle'),$content);
            return $content;
        });
    ...
    public function checkPrivileges(){
    	...
    	if($isProVisit!=''){
    		...
    		$userType = -1;
    		if((int)session('tb_USER.userId')>0)$userType = 0;
    		if((int)session('tb_USER.shopId')>0)$userType = 1;
    		//未登录不允许访问受保护的资源
    		if($userType==-1){
    			if($request->isAjax()){
    ...
    public function checkAdminAuth(){
        ...
        if(empty($STAFF)){
            ...
            $privileges = session('tb_STAFF.privileges');
            $visit = strtolower("/".$request->path());
